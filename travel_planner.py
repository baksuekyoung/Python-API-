...
# 5. 메인 실행 흐름
# ------------------------------------------------------------------
def get_mock_recommendation() -> dict:
    """API 키 없이 구조를 테스트해보기 위한 가짜(샘플) 1차 추천 데이터."""
    return {
        "recommended_city": "청주",
        "weather": "3월 중순 평균 12~16°C, 아침저녁으로 쌀쌀함",
        "events": ["청주 국제공예비엔날레(시기별 상이)", "북페어/도서 관련 지역 행사"],
        "reason": "청주는 '기적의 도서관' 등 작은도서관 정책의 발상지로 잘 알려져 있어 "
                   "도서관 여행 테마와 잘 맞습니다. (이 값은 --mock 옵션의 샘플 데이터입니다)",
    }


def get_mock_libraries() -> list:
    """API 키 없이 구조를 테스트해보기 위한 가짜(샘플) 작은도서관 목록."""
    return [
        {
            "name": "(샘플) 청주 상당작은도서관",
            "address": "충청북도 청주시 상당구 어딘가 1",
            "category": "문화,예술 > 도서관 > 작은도서관",
...
def build_report_mock(date_str: str, recommendation: dict, libraries: list, errors: list) -> str:
    """API 호출 없이 로컬에서 조립하는 샘플 리포트 (--mock 전용)."""
    library_text = (
        "\n".join(f"- {lib['name']} ({lib['category']}) - {lib['address']}" for lib in libraries)
        if libraries else "데이터 없음"
    )
    events_text = ", ".join(recommendation.get("events", [])) or "정보 없음"
    error_section = "\n## 오류 요약(errors)\n"
    error_section += "- 없음 (--mock 모드: 실제 API 호출을 하지 않았습니다)\n"

    return (
        f"# {date_str} 작은도서관 여행 추천 리포트 (MOCK 샘플)\n\n"
        f"## 추천 지역\n{recommendation.get('recommended_city')}\n\n"
        f"## 추천 이유\n{recommendation.get('reason')}\n\n"
        f"## 날씨 요약\n{recommendation.get('weather')}\n\n"
        f"## 행사/축제\n{events_text}\n\n"
        f"## 작은도서관 추천\n{library_text}\n\n"
        f"## 1일 일정 제안\n"
        f"- 오전: 대표 작은도서관 방문, 큐레이션 서가 둘러보기\n"
        f"- 오후: 인근 카페에서 독서, 두 번째 도서관 방문\n"
        f"- 저녁: 지역 맛집에서 식사 후 숙소 체크인\n"
        + error_section
    )
...
def main():
    ...
    parser.add_argument(
        "--mock",
        action="store_true",
        help="실제 API를 호출하지 않고 샘플 데이터로 전체 흐름만 테스트합니다. "
             "API 키가 아직 없을 때 구조를 먼저 확인해보는 용도입니다.",
    )
    ...
    if args.mock:
        print("=" * 50)
        print(" [MOCK 모드] 실제 API를 호출하지 않고 샘플 데이터로 진행합니다.")
        print("=" * 50)
        print(f"[1/3] 1차 추천 생성 중(샘플)...")
        recommendation = get_mock_recommendation()
        print(f"   - recommended_city: \"{recommendation.get('recommended_city')}\"")

        print(f"[2/3] 작은도서관 검색 중(샘플)...")
        libraries = get_mock_libraries()
        print(f"   - 작은도서관 {len(libraries)}곳 (샘플)")

        print(f"[3/3] 최종 리포트 생성 중(샘플)...")
        report_md = build_report_mock(date_str, recommendation, libraries, errors)
        print(f"   - 리포트 생성 완료")
    else:
        check_api_keys()
